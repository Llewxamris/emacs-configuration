...
#  LocalWords:  shatteringly Hideshow
...
#+BEGIN_SRC emacs-lisp
  (use-package general
    ...
    (general-create-definer max-org-leader-def
      :prefix "SPC o")

    (general-create-definer max-local-leader-def
      :prefix "SPC m")
...
#+BEGIN_SRC emacs-lisp :tangle lib/max.el :mkdirp yes
  ...
  (defconst max-font-sizes-alist '((small . 9.5) (large . 12))
  ...
  (defun max--add-pretty-symbols-to-alist (initial-alist seq-elm)
    "Flatten SEQ-ELM and add the result to INITIAL-ALIST."
    (let ((symbol (car seq-elm))
          (char (cdr seq-elm)))
      (if (listp symbol)
          (map-merge 'list initial-alist
                     (seq-reduce
                      (lambda (init sym)
                        (map-insert init sym char))
                      symbol '()))
        (map-insert initial-alist symbol char))))

  (defmacro max-define-pretty-symbols (&rest body)
    "Set `prettify-symbols-alist' with the values of BODY.

  BODY must be a list of the form ([SYMBOL SYMBOLS] CHARACTER...)'
  where SYMBOL is a string, SYMBOLS is a list of strings, and
  CHARACTER is the character to replace the string(s).

  Providing SYMBOL adds the element (SYMBOL . CHARACTER) to
  `prettify-symbols-alist'. Providing SYMBOLS adds an element
  (SYMBOL . CHARACTER)--where SYMBOL is an element of SYMBOL--per
  element of SYMBOLS.

  For example:
  (max-define-pretty-symbols \"FOO\" \"BAZ\") adds the
  element (\"FOO\" . \"BAZ\") to prettify symbols,
  while (max-define-pretty-symbols (\"FOO\" \"BAR\") \"BAZ\") adds the elements
  \(\"FOO\" . \"BAZ\") and (\"BAR\" . \"BAZ\") to prettify symbols."
    `(let* ((symbol-tuple-list
            (seq-partition (quote ,body) 2))
           (local-prettify-symbols-alist
            (seq-reduce
             (lambda (alist tuple)
               (max--add-pretty-symbols-to-alist alist tuple))
             symbol-tuple-list ())))
       (setq prettify-symbols-alist local-prettify-symbols-alist)))

  (defmacro max-define-company-backends (&rest backends)
    "Set `company-backends' to BACKENDS."
...
#+BEGIN_SRC emacs-lisp
  (use-package emacs
    ...
    (max-leader-def
      :states 'normal
      "n" '(:ignore :which-key "narrowing prefix")
      "n f" #'narrow-to-defun
      "n F" #'narrow-to-defun-include-comments
      "n p" #'narrow-to-page
  ...
  If the file is already open in another buffer, switch to that buffer."
    ...
    ;; Trigger garbage collection whenever I switch focus from Emacs to another
    ;; window/application.
    ...
    ;; Draw the underline at the decent line. The descent line being the point
    ;; where the descenders of glyphs like 'g' and 'y' end.
    (x-underline-at-descent-line t)

    ;; Hide the mouse pointer when typing.
    (make-pointer-invisible t)

    ;; When scrolling quckly through a buffer, only fontify the portion of the
    ;; buffer that will be displayed. Helps *A LOT* on Windows.
...
#+BEGIN_SRC emacs-lisp
  (use-package files
    ...
    ;; Do not ask for confirmation on either opening an non-existent file
    ;; (creating it), or a non-existent buffer (creating it without an associated
    ;; file).
...
#+BEGIN_SRC emacs-lisp
  (use-package minibuf-eldef
    ...
    :custom
    ;; Shorten "(default ...)" to "[...]" in the minibuffer.
...
#+BEGIN_SRC emacs-lisp
  (use-package evil-collection
    ...
    :custom
    ;; Mirror the behaviour of Vim's "YouCompleteMe" plugin.
    (evil-collection-company-use-tng t)
    ;; Use Evil bindings in the minibuffer.
    (evil-collection-setup-minibuffer t))
#+END_SRC

~evil-commentary~ is a port of the Vim plugin ~vim-commentary~. This lets me use
=gc= to comment out motions of text, or =gcc= to comment an entire line.
...
#+BEGIN_SRC emacs-lisp
  (use-package ibuffer
    ...
    (max-leader-def
      :states 'normal
      "b" '(:ignore t :which-key "buffer manipulation prefix")
...
#+BEGIN_SRC emacs-lisp
  (use-package dired
    ...
    (max-leader-def
      :states 'normal
      "f" '(nil :which-key "file manipulation prefix")
      "f f" #'find-file
      "f j" #'dired-jump
      "f s" #'save-buffer)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package flyspell-correct
    ...
    (max-leader-def
      :states 'normal
      "S" '(nil :which-key "spell-checking prefix")
      "S b" #'flyspell-buffer
...
** Hideshow
The /Hideshow/ package (hs) does exactly what it says on the box. It makes it
possible to hide and show blocks of code (think folding from Vim). I like using
folding on top-level blocks in source code. Seeing the full source code can be
overwhelming and distracting. Folding helps me focus on one thing without having
to /narrow/ the buffer. Not that I do not like narrowing, I just think it has a
different use case.
...
#+BEGIN_SRC emacs-lisp
  (use-package hideshow
    :ensure nil
    :custom
    (hs-isearch-open t "Open comments and code blocks when searching."))
#+END_SRC

** Semantic
Whenever I am programming in a language where using a language server is not
possible/practical, I miss some of the simpler features. Seeing documentation in
the minibuffer, jumping to definitions, etc. Not that Emacs cannot do this, but
it really seems to be hit or miss.

Semantic parses source code to construct a parse tree. It provides an API that
allows users to perform actions with that tree. Like jumping to a specific
declaration, moving up/down call stacks, etc.

#+BEGIN_SRC emacs-lisp
  (use-package semantic
    :ensure nil
    :hook ((c-mode emacs-lisp-mode) . semantic-mode)
    :general
    (max-leader-def
      :states 'normal
      ""    '(:ignore t :which-key "semantic prefix")
      "s b" #'semantic-mrub-switch-tags
      "s j" #'semantic-complete-jump-local
      "s J" #'semantic-complete-jump
      "s g" #'semantic-symref-symbol
      "s G" #'semantic-symref
      "s n" #'senator-next-tag
      "s p" #'senator-previous-tag)
    :custom
...
Semantic Refactor builds on top of Semantic. It exposes a function to refactor
source code by looking at its semantic elements. This lets me do things like
extracting blocks of code into their own functions, renaming variables across
an entire project, etc.

#+BEGIN_SRC emacs-lisp
  (use-package srefactor
    :after semantic
    :general
    (max-local-leader-def
      :states 'normal
      "s r" #'srefactor-refactor-at-point)
    :init
...
#+BEGIN_SRC emacs-lisp
  (use-package c-mode
    ...
    :mode "\\.[ch]\\'"
    :init
    (defun max-c-mode-hook ()
      (hs-minor-mode)
      (max-define-company-backends company-clang
                                   company-semantic
                                   company-capf
                                   company-yasnippet)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package gdb-mi
    ...
    :bind (:map c-mode-base-map
                ("C-c d" . gdb))
    :custom
    ;; Show the source for the "main" function of the program.
    (gdb-show-main t)
    ;; Create an IDE-like interface when running gdb.
    (gdb-many-windows t))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package elisp-mode
    ...
    :hook (emacs-lisp-mode . max--elisp-mode)
    :general
    (max-local-leader-def
      :states 'normal
      "b" #'eval-buffer)
    :init
    (defun max--elisp-mode ()
      (max-define-company-backends company-capf
                                   company-yasnippet
                                   company-files
                                   company-dabbrev-code)
      (max-define-pretty-symbols "lambda" ?λ
                                 "not" ?¬
                                 "and" ?∧
                                 "or" ?∨)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (max-local-leader-def
      ...
      "h"   '(:ignore t :which-key "org-insert-[sub]heading prefix")
      "h i" #'org-insert-heading-respect-content
    ...
    :functions (max-set-prettify-symbols)
    :init
  ...
  alongside hooks for automatic dynamic block updating."
    ...
    (defun max-org-mode-hook ()
      ...
      (auto-fill-mode)
      (max-define-pretty-symbols ("#+BEGIN_SRC" "#+begin_src") ?⚞
                                 ("#+END_SRC" "#+end_src") ?⚟)
      (max-define-company-backends company-ispell
                                   company-yasnippet
                                   company-files))
    ...
    ;; Include the Org Habit tracker module. Without the module, habit tracking
    ;; would not work.
    (add-to-list 'org-modules 'org-habit)

    ;; Define the languages I want to be able to evaluate within an Org source
    ;; block.
